...
export function HomeStats({ ordersValue, ordersDelta, deployments, aiModels }: HomeStatsProps) {
  // Parse ordersValue (e.g., "12.4M" -> 12.4, "24,148" -> 24148)
  const ordersNumeric = parseFloat(ordersValue.replace(/[^0-9.]/g, ""));
  // Extract only letter suffixes (M, K, B), not commas or other punctuation
  const ordersSuffix = ordersValue.replace(/[0-9.,\s]/g, "");
    ...
    <div className="mt-16 grid gap-6 sm:grid-cols-3 w-full max-w-3xl">
      <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/[0.02] p-6 backdrop-blur-sm">
        <div className="flex items-center gap-3 text-emerald-400">
          ...
          <span className="text-sm font-medium">Deliveries Completed</span>
        </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/[0.02] p-6 backdrop-blur-sm">
        <div className="flex items-center gap-3 text-sky-400">
          ...
          <span className="text-sm font-medium">Robots Monitored</span>
        </div>
        <div className="mt-2 text-3xl font-bold">
          <AnimatedCounter 
            value={deployments} 
        ...
        <div className="mt-1 text-sm text-white/50">Real-time telemetry</div>
      </div>
